...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    // FIXED: Fallback to 'checkoutSubtotal' if it exists, ensuring backward navigation doesn't double-calculate fees
  ...
  const grandTotal = cartTotal + deliveryFee + systemFee;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Save structural checkout data to session storage
    sessionStorage.setItem('checkoutForm', JSON.stringify(formData));
    
    // FIXED: Preserve the baseline raw subtotal so coming back to this page doesn't glitch
    sessionStorage.setItem('checkoutSubtotal', cartTotal.toString());
    
    // FIXED: Overwrite checkoutTotal with the explicitly calculated true grand total for the review page
