...
// Fee configuration (for display purposes - farmer sees the total including fees)
...
export default function FarmerOrdersPage() {
  ...
  const filters = [
    ...
    { value: "PENDING", label: "Need Action" },
            ...
            {orders.map((order, orderIdx) => (
              <div key={`order-${order.id}-${orderIdx}`} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-1 rounded-lg text-xs font-bold ${getStatusColor(order.status)}`}>
                        {order.status === 'PENDING' ? 'Awaiting Action' : order.status}
                      </span>
                      <div className="flex gap-2">
                ...
                <div className="p-4">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-2">
                      {order.orderItems.map((item, itemIdx) => {
                        // Calculate item subtotal (this is the farmer's portion without fees)
                        ...
                        return (
                          <div key={`order-${order.id}-item-${item.id}-${itemIdx}`} className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">{formatETB(itemSubtotal)}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  {/* Total - Farmer sees the full order amount including fees */}
                  <div className="flex justify-between items-center pt-3 border-t border-slate-100">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[10px] text-slate-400">(Includes delivery, system fee & tax)</p>
                    </div>
                  </div>

                  {/* Fee Breakdown Summary (for farmer's info) */}
                  {(() => {
                    // Calculate approximate subtotal from order items
                    ...
                    return (
                      <div className="mt-3 p-3 bg-slate-50 rounded-lg">
                        <p className="text-xs font-medium text-slate-600 mb-1">Fee Breakdown:</p>
                        <div className="space-y-1 text-xs text-slate-500">
                          <div className="flex justify-between">
                            <span>Items Subtotal:</span>
                            <span>{formatETB(itemsSubtotal)}</span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between">
                            <span className="flex items-center gap-1">⚙️ System Fee (0.5%):</span>
                            <span>{formatETB(systemFee)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="flex items-center gap-1">💰 Tax (5%):</span>
                            <span>{formatETB(taxFee)}</span>
                          </div>
                          <div className="flex justify-between pt-1 border-t border-slate-200 font-medium">
                            <span>Customer Total:</span>
                            <span className="text-green-600">{formatETB(order.totalAmount)}</span>
                          </div>
                        </div>
                        <p className="text-[10px] text-slate-400 mt-2">
                          Note: Delivery fee, system fee, and tax are added to the customer's total.
                          You receive the items subtotal amount.
                        </p>
                      </div>
                    );
                  })()}
