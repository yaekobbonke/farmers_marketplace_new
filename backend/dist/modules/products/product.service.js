...
class ProductService {
    ...
    static async update(id, userId, data, role) {
    ...
    static async remove(id, userId, role) {
        const product = await prisma_1.default.product.findUnique({ where: { id } });
        if (!product)
            throw new Error("Product not found");
        if (product.farmerId !== userId && role !== "ADMIN") {
        ...
        const deletedProduct = await prisma_1.default.product.delete({ where: { id } });
        const oneMinuteAgo = new Date(Date.now() - 60 * 1000);
        const recentNotification = await prisma_1.default.notification.findFirst({
            where: {
                userId: userId,
    ...
    static async verifyProduct(id) {
        ...
        const verifiedProduct = await prisma_1.default.product.update({
            where: { id },
    ...
    static async rejectProduct(id, reason) {
        ...
        const rejectedProduct = await prisma_1.default.product.delete({ where: { id } });
        const oneMinuteAgo = new Date(Date.now() - 60 * 1000);
        const recentNotification = await prisma_1.default.notification.findFirst({
            where: {
                userId: product.farmerId,
    ...
    static async featureProduct(id) {
        const product = await prisma_1.default.product.findUnique({
            where: { id },
        ...
        const featuredProduct = await prisma_1.default.product.update({
            where: { id },
    ...
    static async updateStock(productId, quantitySold) {
        const product = await prisma_1.default.product.findUnique({
            where: { id: productId },
    ...
    static async getLowStockProducts(userId, threshold = 10) {
        return prisma_1.default.product.findMany({
            where: {
                farmerId: userId,
    ...
    static async getTopViewedProducts(limit = 10) {
        try {
    ...
    static async getProductAnalytics(userId, period = "week") {
        if (!userId || isNaN(userId)) {
            throw new Error("Invalid user ID");
        }
    ...
    static async generateDailyViewData(userId, days) {
        const dailyData = [];
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        for (let i = 0; i < days; i++) {
            ...
            const productsCreated = await prisma_1.default.product.count({
                where: {
                    farmerId: userId,
                    createdAt: {
                        gte: date,
            ...
            const productsWithViews = await prisma_1.default.product.findMany({
                where: {
                    farmerId: userId,
                    createdAt: {
                        gte: date,
